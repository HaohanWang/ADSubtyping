...
def train_single_epoch(config, model, dataloader, criterion, optimizer, pgd_attack,
    ...
    for i, data in tbar:
        ...
        if pgd_attack:
            ...
            loss = criterion(outputs, labels) + config.pgd.loss.alpha*torch.norm(outputs-pgd_outputs,dim=1).pow(2).sum()
        else:
            loss = criterion(outputs, labels)
...
def run(config):
    ...
    pgd_attack = None
    if config.pgd.use:
        pgd_attack = torchattacks.PGD(model, eps=config.pgd.eps, alpha=config.pgd.alpha, iters=config.pgd.iters, random_start=True)
    ...
    train_aug = T.Compose([MinMaxNormalization(), T.Lambda(lambda x : x + 0.000225*torch.randn_like(x)), MinMaxNormalization()])
    val_aug = MinMaxNormalization()
    dataloaders = {}
    dataloaders['train'] = get_dataloader(config, 'train', config.data.params.weighted_sampler, train_aug)
    dataloaders['val'] = get_dataloader(config, 'val', False, val_aug)
