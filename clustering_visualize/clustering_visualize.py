...
def df2path(df, i):
    ...
    return join(BASE_DIR + 'new_saliency_ckp2', df.split.iloc[i], df.participant_id.iloc[i],
                df.session_id.iloc[i] + '.npy')
...
if __name__ == '__main__':
    ...
    train_df = pd.read_csv(BASE_DIR + 'new_saliency_ckp2/train_saliency_info.csv')
    # test_df = pd.read_csv(BASE_DIR + 'new_saliency_maps_ckp1/test_saliency_info.csv')
    val_df = pd.read_csv(BASE_DIR + 'new_saliency_ckp2/val_saliency_info.csv')
    ...
    for p, l in tqdm(zip(snp_correct_paths, snp_correct_labels)):
        print(p)
        print(l)
        drawFromPath(p,l, BASE_DIR + '2d_new_saliency_ckp2_smoothing')
